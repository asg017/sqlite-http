...
let path: string;
try {
  const customPath = Deno.env.get("DENO_SQLITE_HTTP_PATH");
  if (customPath) path = customPath;
  else {
    path = await download({
      url: {
        darwin: {
          x86_64: `${BASE}/deno-darwin-x86_64.http0.dylib`,
        },
        windows: {
          x86_64: `${BASE}/deno-windows-x86_64.http0.dll`,
        },
        linux: {
          x86_64: `${BASE}/deno-linux-x86_64.http0.so`,
        },
      },
      suffixes: {
        darwin: "",
  ...
  const error = new Error("Failed to load sqlite-http extension");
  error.cause = e;
...
/**
 * Returns the full path to the compiled sqlite-http extension.
 * Caution: this will not be named "http0.dylib|so|dll", since plug will
 ...
 */
export const entrypoint = "sqlite3_http_init";

// TODO
export const entrypointNoNetwork = "sqlite3_http_no_network_init";
...
// TODO
export function loadNoNetwork(db: Db): void {
  db.loadExtension(path, entrypointNoNetwork);
}
